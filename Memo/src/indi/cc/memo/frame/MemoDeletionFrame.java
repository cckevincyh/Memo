...
public class MemoDeletionFrame extends JDialog{
	...
	private List<MemoBean> memoBeans;	//查询结果的Lsit集合
	private int id;	//当前备忘录的id号
	/**
	 * 
	 * @param owner 它的父窗口
	 * @param title 窗口名
	...
	public MemoDeletionFrame(JFrame owner, String title, boolean modal){ 
        ...
        deleteButton = new JButton("删除");// 创建“删除”按钮
        //“删除”按钮事件监听
        deleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				memoBean.setId(id);
				JdbcHelper.delete(memoBean);//从数据库中移除
				memoBeans.remove(index);	//从集合中移除
				if(index>0){
					index--;
				}
				if(memoBeans.size()==0){//没有数据了
					titleTextField.setText("");
					memotimeTextField.setText("");
					usernameTextField.setText("");
					contentTextArea.setText("");
					memotypeTextField.setText("");
					JOptionPane.showMessageDialog(jd, "备忘录为空！","", JOptionPane.WARNING_MESSAGE);
					return ;
				}else{//否则，显示上一条记录
					updateContent(index);
					 JOptionPane.showMessageDialog(jd, "备忘录删除成功！", "", JOptionPane.WARNING_MESSAGE);
				}
        ...
        memoBeans = JdbcHelper.queryAll();	//查询全部
